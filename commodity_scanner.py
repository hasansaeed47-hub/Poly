...
def get_live_prices() -> dict[str, PriceData]:
    """Real-time commodity prices as of March 13, 2026 market session.
    Prices sourced from live web search and Vantage platform data.
    NOTE: Vantage uses USD/bushel for wheat (not cents/bushel like CBOT).
    """
    return {
        "CL": PriceData(
            symbol="CL", name="WTI Crude Oil",
            current_price=96.10, price_unit="USD/barrel",
            day_change_pct=9.72, week_change_pct=17.28,
            month_change_pct=51.65, year_change_pct=39.0,
            day_high=97.16, day_low=88.89, prev_close=87.25,
            fifty_two_week_high=119.48, fifty_two_week_low=54.98,
            vantage_symbol="USOUSD", vantage_available=True,
            timestamp="2026-03-13T00:00:00Z",
        ),
        "GC": PriceData(
            symbol="GC", name="Gold",
            current_price=5138.11, price_unit="USD/oz",
            day_change_pct=-0.79, week_change_pct=-2.5,
            month_change_pct=3.0, year_change_pct=100.0,
            day_high=5179.10, day_low=5107.72, prev_close=5179.10,
        ...
        "SI": PriceData(
            ...
            current_price=83.79, price_unit="USD/oz",
            day_change_pct=-4.20, week_change_pct=-8.5,
            month_change_pct=-2.0, year_change_pct=161.0,
            day_high=85.10, day_low=79.65, prev_close=84.37,
            fifty_two_week_high=121.67, fifty_two_week_low=28.16,
            vantage_symbol="XAGUSD", vantage_available=True,
            timestamp="2026-03-13T00:00:00Z",
        ),
        "HG": PriceData(
            symbol="HG", name="Copper",
            current_price=5.885, price_unit="USD/lb",
            day_change_pct=-0.40, week_change_pct=0.8,
            month_change_pct=4.5, year_change_pct=42.0,
            day_high=5.919, day_low=5.876, prev_close=5.909,
            fifty_two_week_high=6.583, fifty_two_week_low=4.030,
            vantage_symbol="COPPER-C", vantage_available=True,
            timestamp="2026-03-13T00:00:00Z",
        ),
        "NG": PriceData(
            symbol="NG", name="Natural Gas",
            current_price=3.212, price_unit="USD/MMBtu",
            day_change_pct=0.09, week_change_pct=5.5,
            month_change_pct=12.0, year_change_pct=15.0,
            day_high=3.292, day_low=3.162, prev_close=3.209,
        ...
        "ZW": PriceData(
            ...
            current_price=5.857, price_unit="USD/bushel",
            day_change_pct=-0.27, week_change_pct=-1.9,
            month_change_pct=15.0, year_change_pct=9.69,
            day_high=5.873, day_low=5.842, prev_close=5.873,
            fifty_two_week_high=6.50, fifty_two_week_low=4.80,
            vantage_symbol="Wheat-Cr", vantage_available=True,
            timestamp="2026-03-13T00:00:00Z",
        ),
        "BRN": PriceData(
            symbol="BRN", name="Brent Crude Oil",
            current_price=100.11, price_unit="USD/barrel",
            day_change_pct=8.84, week_change_pct=15.0,
            month_change_pct=48.0, year_change_pct=36.0,
            day_high=101.53, day_low=92.89, prev_close=91.98,
            fifty_two_week_high=119.50, fifty_two_week_low=58.40,
...
class SentimentAnalyzer:
    ...
    SENTIMENT_DB = {
        ...
        "ZW": [
            SentimentData("Price Action", "NEUTRAL", 0.30,
                          "Pulled back from $5.94 high to $5.857 — consolidating above MAs"),
            SentimentData("Fund Flows", "BULLISH", 0.70,
                          "Funds bought 31,000 SRW contracts recently — short covering"),
            ...
            SentimentData("TradingView", "BULLISH", 0.60,
                          "Buy rating; technicals supportive but momentum slowing"),
        ...
        "SI": [
            SentimentData("Finance Magnates", "BEARISH", -0.60,
                          "XAG/USD crashed 13% in 2 days below $84 — massive selloff"),
            SentimentData("Barchart", "BULLISH", 0.50,
                          "Long-term posture bullish; but near-term severely damaged"),
            SentimentData("COT Data", "NEUTRAL", 0.10,
                          "Cautious positioning despite bullish fundamentals"),
            SentimentData("Gold/Silver Ratio", "BULLISH", 0.40,
                          "Ratio at 61.3 — silver undervalued vs gold historically"),
            SentimentData("YoY Performance", "BULLISH", 0.70,
                          "Up 161% year-over-year despite the crash"),
        ],
    }
...
class TradeScanner:
    ...
    def _get_catalysts(self, symbol: str) -> list[str]:
        catalysts_db = {
            ...
            "ZW": [
                "Holding near $5.86 after pullback — consolidating above MAs",
                ...
                "ING/JPM bullish on wheat for 2026",
            ],
            "NG": [
            ...
            "SI": [
                "Up 161% year-over-year — strongest metal performer",
                "Crashed 13% in 2 days — potential oversold bounce",
                "Gold/silver ratio at 61.3 — silver undervalued vs gold",
            ],
        }
    ...
    def _get_risks(self, symbol: str) -> list[str]:
        risks_db = {
            "CL": [
                "RSI at 82 — deeply overbought; violent mean-reversion possible",
            ...
            "ZW": [
                ...
                "Pulled back from $5.94 highs — momentum stalling near term",
                "Currently in your portfolio at a loss — watch $5.70 support",
            ],
            "NG": [
                "Technically weakest commodity — below 200-day MA",
            ...
            "SI": [
                "Crashed 13% in 2 days — catching a falling knife risk",
                "Down 8.5% on the week — bearish momentum accelerating",
                "Could test $79.50 support before bouncing",
