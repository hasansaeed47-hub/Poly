...
//! SL: bid <= 50% of fill (single confirm, no CL flip required)
//! Tick: 500ms during active, 1s idle
//! Fixes over original: exact CL open/close snap, CSV logging, consec loss guard
...
const CSV_PATH: &str = "sniper_9mar_trades.csv";
...
impl State {
    ...
    /// Exact CL at timestamp — zero tolerance, exact second only
    fn cl_exact(&self, a: &str, t: i64) -> Option<f64> {
        let s = self.snap.get(a)?;
        s.get(&t).copied()
    }
    fn cl_latest(&self, a: &str) -> Option<f64> { self.cl.get(a).copied() }
    /// Find CL snap closest to target timestamp (for continuous refinement)
    fn cl_closest(&self, a: &str, t: i64) -> Option<(i64, f64)> {
        let s = self.snap.get(a)?;
        s.iter().min_by_key(|(&ts, _)| (ts - t).abs()).map(|(&ts, &px)| (ts, px))
    }
...
impl Scan {
    fn new()->Self { Scan { http:reqwest::Client::builder().user_agent("cls/9").timeout(Duration::from_secs(5)).build().expect("h"), cache:Vec::new(), last:Instant::now()-Duration::from_secs(999) } }
...
impl BkC {
    fn new()->Self { BkC { http:reqwest::Client::builder().user_agent("cls/9").timeout(Duration::from_secs(2)).build().expect("h"), c:HashMap::new(), t:HashMap::new() } }
...
impl Tracker {
    fn new(cfg: EngCfg) -> Self {
        Tracker { cfg, cap: 100.0, w:0, l:0, sl:0, pnl:0.0, consec_loss:0,
    ...
    fn record_win(&mut self, p: f64) { self.w+=1; self.pnl+=p; self.consec_loss=0; }
    fn record_loss(&mut self, p: f64) { self.l+=1; self.pnl+=p; self.consec_loss+=1; }
    fn record_sl(&mut self, p: f64) { self.sl+=1; self.pnl+=p; self.consec_loss+=1; }
    fn consec_ok(&self) -> bool { self.consec_loss < MAX_CONSEC }
}
...
struct Sniper {
    ...
    // CL open: exact snap at window start_ts, continuously refined to closest
    cl_opens: HashMap<String, f64>,
    cl_open_ts: HashMap<String, i64>,
    // CL close: exact snap at window end_ts, continuously refined to closest
    cl_closes: HashMap<String, f64>,
    cl_close_ts: HashMap<String, i64>,
    csv: CsvLog,
    start: Instant,
}

impl Sniper {
    fn new(st: SS, scan: Scan, bk: BkC, csv: CsvLog) -> Self {
        ...
        Sniper { st, scan, bk, trackers, cl_opens: HashMap::new(), cl_open_ts: HashMap::new(),
                 cl_closes: HashMap::new(), cl_close_ts: HashMap::new(), csv, start: Instant::now() }
    }

    fn has_active(&self) -> bool { self.trackers.iter().any(|t| t.active.is_some()) }

    /// CL open — continuously updated to snap closest to start_ts (exact preferred)
    async fn record_opens(&mut self, windows: &[Win]) {
        let s = self.st.read().await;
        for w in windows {
            // Try exact match at start_ts first
            if let Some(px) = s.cl_exact(w.asset, w.start_ts) {
                if px > 0.0 {
                    // Exact match — always use it, distance = 0
                    let prev_dist = self.cl_open_ts.get(&w.slug)
                        .map(|&ts| (ts - w.start_ts).abs())
                        .unwrap_or(i64::MAX);
                    if prev_dist > 0 {
                        self.cl_opens.insert(w.slug.clone(), px);
                        self.cl_open_ts.insert(w.slug.clone(), w.start_ts);
                    }
                    continue;
                }
            }
            // Fallback: closest snap to start_ts
            if let Some((best_ts, best_px)) = s.cl_closest(w.asset, w.start_ts) {
    ...
    /// CL close — continuously updated to snap closest to end_ts (exact preferred)
    async fn record_closes(&mut self, windows: &[Win]) {
        let s = self.st.read().await;
        let now = Utc::now().timestamp();

        // Collect targets: windows that have ended + active trades that have ended
        ...
        for tr in &self.trackers {
            if let Some((slug, pt)) = &tr.active {
                if now > pt.end_ts {
                    targets.push((slug.clone(), pt.asset, pt.end_ts));
                }
            }
        }

        for (slug, asset, end_ts) in targets {
            // Try exact match at end_ts first
            if let Some(px) = s.cl_exact(asset, end_ts) {
                if px > 0.0 {
                    let prev_dist = self.cl_close_ts.get(&slug)
                        .map(|&ts| (ts - end_ts).abs())
                        .unwrap_or(i64::MAX);
                    if prev_dist > 0 {
                        self.cl_closes.insert(slug.clone(), px);
                        self.cl_close_ts.insert(slug, end_ts);
                    }
                    continue;
                }
            }
            // Fallback: closest snap to end_ts
            if let Some((best_ts, best_px)) = s.cl_closest(asset, end_ts) {
                if best_px <= 0.0 { continue; }
    ...
    async fn tick(&mut self) {
        ...
        // Record CL opens — exact at start_ts, continuously refined
        self.record_opens(&wins).await;

        // Record CL closes — exact at end_ts, continuously refined
        self.record_closes(&wins).await;
        ...
        for tr in &mut self.trackers {
            ...
            if tr.active.is_some() {
                ...
                let bk = self.bk.get(&pt_tid);
                if bk.hb && bk.bb <= pt_sl_px {
                    let recovery = pt_shares * (bk.bb - SLIP).max(0.0);
                ...
                // Settlement
                if now >= pt_end_ts + 3 {
                    // Use exact CL close from record_closes, NO fallback to cl_latest
                    let cl_close = self.cl_closes.get(&slug).copied().unwrap_or(0.0);

                    // CLOB cross-check
                    let tid_up = wins.iter().find(|w| w.slug == slug).map(|w| w.tid_up.clone()).unwrap_or_default();
                    let tid_dn = wins.iter().find(|w| w.slug == slug).map(|w| w.tid_dn.clone()).unwrap_or_default();
                    let bk_up = self.bk.get(&tid_up);
                    let bk_dn = self.bk.get(&tid_dn);

                    let cl_dir = if pt_cl_open>0.0 && cl_close>0.0 {
                        Some(if cl_close>=pt_cl_open {"UP"} else {"DOWN"})
                    } else { None };
                    ...
                    // CL is truth — only use CLOB if CL unavailable
                    let actual = cl_dir.or(clob_dir);

                    if let Some(actual) = actual {
                        let won = actual == pt_dir;
                        let pnl = if won { pt_shares * 1.0 - STAKE } else { -STAKE };
                        let event = if won { "WIN" } else { "LOSS" };
                        if won {
                            info!("[{}] WIN {} {} @{:.3} ${:+.2}", tr.cfg.id, pt_asset.to_uppercase(), pt_dir, pt_px, pnl);
                            tr.record_win(pnl);
                        } else {
        ...
        self.cl_open_ts.retain(|k,_| k.rsplit('-').next().and_then(|s|s.parse::<i64>().ok()).map(|t|t>cut).unwrap_or(false));
        self.cl_closes.retain(|k,_| k.rsplit('-').next().and_then(|s|s.parse::<i64>().ok()).map(|t|t>cut).unwrap_or(false));
        self.cl_close_ts.retain(|k,_| k.rsplit('-').next().and_then(|s|s.parse::<i64>().ok()).map(|t|t>cut).unwrap_or(false));
        for tr in &mut self.trackers {
...
async fn main() -> Result<()> {
    ...
    info!("================================================================");
    info!("  CL SNIPER — 9th March 2026 (fixed)");
    ...
    info!("  SL: bid<=50% single confirm | Tick: 500ms/1s adaptive");
    info!("  Regime: 1h range < {}% = CHOP, skip", REGIME_THRESH);
    info!("  Fixes: exact CL snap, CSV logging, consec loss guard");
    info!("================================================================");

    let csv = CsvLog::new().context("CSV init")?;
    ...
    let mut sniper = Sniper::new(st.clone(), Scan::new(), BkC::new(), csv);
