...
from config import (TIERS,EDGE_15MIN,EDGE_MAKER,EDGE_TAKER,MAX_STAKE,DAILY_BUDGET,
...
class ExecDecider:
    ...
    def edge_th(self,tf:TF,mode:ExecMode)->float:
        if tf==TF.M15:return EDGE_15MIN
...
class Executor:
    ...
    async def enter(self,m:Market,direction:Dir,size:float,price:float,mode:ExecMode)->Optional[Position]:
        ...
        if mode==ExecMode.MAKER:
            oid=await self.api.place(m.id,side,price,shares)
        else:
            oid=await self.api.market_order(m.id,side,shares)
    ...
    async def exit(self,pos:Position,pct:float=1.0)->bool:
        ...
        oid=await self.api.market_order(pos.market.id,"SELL",shares)
