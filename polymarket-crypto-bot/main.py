...
from config import get_creds,ASSETS,EDGE_15MIN,EDGE_TAKER
...
class Bot:
    def __init__(self):
        ...
        self.pf=PriceFeed()
        self.api=PolyAPI()
        self.scanner=Scanner(self.api,30)
    ...
    async def init(self):
        log.info(f"=== POLYMARKET BOT ({'PAPER' if self.paper else 'LIVE'}) ===")
        await self.api.init()
        self.db.init()
    ...
    async def _scan(self):
        ...
        async def proc_market(m):
            ...
            book=await self.api.get_book(m.id)
            sig=self.model.calc(m,p,book)
            
            th=EDGE_15MIN if m.tf==TF.M15 else EDGE_TAKER
            if sig.edge<th:return None
            return sig
    ...
    async def _process(self,sig,score):
        ...
        log.info(f"ENTRY: {m.asset.value} {m.tf.value} | Edge:{sig.edge:.1%} | ${sz.final} | {ed.mode.value}")
    ...
    async def _on_close(self,t):
        log.info(f"CLOSED: {t.asset.value} | PnL:${t.pnl:+.2f} | {t.reason}")
