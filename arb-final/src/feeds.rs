/// feeds.rs — All external data feeds (standalone copy for arb-final)
///
/// Three feeds:
...
/// 3. BN price feed   : WebSocket from Binance aggTrade (diagnostics only)
/// 4. Market discovery: Batched REST via Gamma API (slug -> token IDs)
...
mod tests {
    ...
    fn build_slug_correct() {
        assert_eq!(build_slug("btc", 15, 1772788200), "btc-updown-15m-1772788200");
        assert_eq!(build_slug("eth", 60, 1772788200), "eth-updown-60m-1772788200");
    }

    #[test]
    fn current_window_starts_15m() {
        let starts = current_window_starts(15, 1772788230);
        assert_eq!(starts[0], 1772787600);
        assert_eq!(starts[1], 1772788500);
    }

    #[test]
    fn current_window_starts_60m() {
        let starts = current_window_starts(60, 1772788230);
        // 1772788230 / 3600 = 492441 → 492441 * 3600 = 1772787600
        assert_eq!(starts[0], 1772787600);
        assert_eq!(starts[1], 1772791200);
    }
